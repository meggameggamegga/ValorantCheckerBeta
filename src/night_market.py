...
def image_skins_night_market(lang,skin_full=None):
    #skin_full = {'Онимару Куницуна': ['901abab9-4a44-09f1-b892-9887a587f079','1000VP','2000VP'],
    #           'Sheriff "Конец игры"': ['443f5a29-4d5e-d63e-a3c6-ada7713172fa','1000VP','2200VP'],
    #           'Stinger "Гроза оборотней"': ['7ffbef55-4108-945e-6bc9-7b897f386d1b','1000VP','2500VP'],
    #           'Bulldog "Radiant Entertainment System"':['7deb8bd7-4ff6-64d2-1f23-2fb7143a5d98','1000VP','2100VP'],
    #           'Нож "Боец"': ['21d49b4a-49ce-79f9-4e08-67959f8d17b5','1500VP','2100VP'],
    #           'Judge "Аквапринт"': ['2a25f841-4769-6536-ded3-8890ee26c430','1200VP','2100VP']}
    ...
    text_position = (800, background_height -35)
    ...
    for k, url in skin_full.items():
        ...
        if url_link:
            ...
            if os.path.exists(image_path):
                ...
                draw_new_price_skin  = ImageDraw.Draw(image_with_background)
                draw_line = ImageDraw.Draw(image_with_background)
                #if len(skin_full) > 100:
                font_name = ImageFont.truetype("ariblk.ttf", 15)
                font_new_price = ImageFont.truetype('ariblk.ttf', 17)
                font_price = ImageFont.truetype('ariblk.ttf',12)
                font_line = ImageFont.truetype('ariblk.ttf',9)
                text_color = (255, 255, 255, 255)  # Цвет текста (черный)

                draw_line.text((245,27),'-------------',fill=text_color,font=font_line)
                draw_price_skin.text((225,5),new_price,fill=text_color,font=font_new_price)
                draw_name_skin.text((10, 120), name, fill=text_color, font=font_name)
                draw_new_price_skin.text((245,24),price,fill=text_color,font=font_price)
...
#image_skins_night_market(lang='RU')
