...
async def admin_start(message: types.Message):
    await message.answer('Привет', reply_markup=auth_menu('Admin'))
...
async def get_photo_sender(message: types.Message, state: FSMContext):
    ...
    await bot.send_photo(chat_id=config.ADMIN_ID, photo=media,
                         caption=f'Вы уверен что хотите сделать рассылку с таким сообщением\n\n'
                                 f'{message}', reply_markup=auth_menu('messages'), parse_mode='HTML')
