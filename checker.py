    ...
    async def autheticate(self):
        ...
        if not authenticate:
            ...
            if authenticate['auth'] == 'response':
                ...
                return {'access_token':self.access_token,'token_id':self.token_id,'entitlements_token':self.entitlements_token,
...
#---------------Получить нужные вещи-----------------#
    ...
    def get_store(self,ent_token=None,acc_token=None,region=None,puuid=None):
        headers = {
            'X-Riot-ClientVersion': "release-08.00-shipping-14-2191955", #"release-07.06-shipping-4-983570",
            "X-Riot-Entitlements-JWT": ent_token,
            "Authorization": f"Bearer {acc_token}"
        }
        # Добавить список items скинов по которым надо сделать запроос
    ...
    def get_my_account(self,ent_token=None,acc_token=None,region=None,puuid=None):
        ...
        headers = {
            ...
            'X-Riot-ClientVersion': "release-08.00-shipping-14-2191955", #"release-08.00-shipping-14-2191955"
            'X-Riot-Entitlements-JWT':ent_token,
            'Authorization': f'Bearer {acc_token}'
        }
        ...
        request = requests.get(
            ...
            headers=headers)
        try:
            rank = request.json()['QueueSkills']["competitive"]['SeasonalInfoBySeasonID'][
                "ec876e6c-43e8-fa63-ffc1-2e8d4db25525"]['CompetitiveTier']  # Seson ID mb change #03dfd004-45d4-ebfd-ab0a-948ce780dac4
            ...
            try:
                ...
                last_match ='На аккаунте не открыт рейтинг'
                #formatted_time = datetime.fromtimestamp(last_match / 1000).strftime('%Y-%m-%d %H:%M:%S')
                return {'VP': VP, 'Kingdom': Kingdom, 'RadiantPoints': RP,'Rank':None,'last_match': last_match,'games':rank}
...
async def main():
    client = ClientAcc(username='meggamegga',password='megamaster70')
    await client.start()
